...
import { tokenClassMap } from "../../backend/src/nft_classes/token_class_definitions";
...
describe("Simple NFT scenario", () => {
  ...
  it("Curator should create Fragment Fungible Token Class", async () => {
    // Given
    const fragmentTokenDto: CreateTokenClassDto =
      await createValidDTO<CreateTokenClassDto>(CreateTokenClassDto, {
        decimals: 2,
        tokenClass: tokenClassMap.LEFC,
        name: "LE Fragment Chunks",
        symbol: "LEFC",
        description:
          "Token for Last Expedition Fragment Fungible In-Game Currency",
        isNonFungible: false,
        image:
    ...
    const response = await client.assets.submitTransaction<TokenClassKey>(
      "CreateTokenClass",
      fragmentTokenDto.signed(client.assets.privateKey),
      TokenClassKey,
    );

    // Then
    expect(response).toEqual(transactionSuccess(tokenClassMap.LEFC));
  });
  it("Curator should create Core Fungible Token Class", async () => {
    // Given
    const coreTokenDto: CreateTokenClassDto =
      await createValidDTO<CreateTokenClassDto>(CreateTokenClassDto, {
        decimals: 2,
        tokenClass: tokenClassMap.LECC,
        name: "LE Core Chunks",
        symbol: "LECC",
        description: "Token for Last Expedition Core Fungible In-Game Currency",
        isNonFungible: false,
        image:
          "https://files.slack.com/files-tmb/T018SUFPRNU-F06JNH22RE0-c43278d8ca/nft_cores_360.gif",
        maxSupply: new BigNumber(maxSupply),
    ...
    const response = await client.assets.submitTransaction<TokenClassKey>(
      "CreateTokenClass",
      coreTokenDto.signed(client.assets.privateKey),
      TokenClassKey,
    );

    // Then
    expect(response).toEqual(transactionSuccess(tokenClassMap.LECC));
  });
  it("Curator should create Mineral Fungible Token Class", async () => {
    // Given
    const mineralTokenDto: CreateTokenClassDto =
      await createValidDTO<CreateTokenClassDto>(CreateTokenClassDto, {
        decimals: 2,
        tokenClass: tokenClassMap.LEMIN,
        name: "LE Minerals",
        symbol: "LEMIN",
        description:
          "Token for Last Expedition Mineral Fungible In-Game Currency",
        isNonFungible: false,
        image:
          "https://files.slack.com/files-tmb/T018SUFPRNU-F06JNH22RE0-c43278d8ca/nft_minerals_360.gif",
        maxSupply: new BigNumber(maxSupply),
      });
    ...
    const response = await client.assets.submitTransaction<TokenClassKey>(
      "CreateTokenClass",
      mineralTokenDto.signed(client.assets.privateKey),
      TokenClassKey,
    );

    // Then
    expect(response).toEqual(transactionSuccess(tokenClassMap.LEMIN));
  });
